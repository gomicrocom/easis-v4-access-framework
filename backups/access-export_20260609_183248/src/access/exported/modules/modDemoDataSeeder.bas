...
Private Const TBL_ORD_ORDER As String = "ord_order"
Private Const TBL_ORD_ORDER_LINE As String = "ord_order_line"
...
Public Sub SeedDemoSalesOrder()
    ...
    If Not modOrderRepository.EnsureOrderRepositoryReady() Then
        Err.Raise vbObjectError + 760, MODULE_NAME, "Order repository is not ready."
    End If

    RequireTable db, TBL_ADR_ADDRESS
    RequireTable db, TBL_ORD_ORDER
    RequireTable db, TBL_ORD_ORDER_LINE
    RequireField db, TBL_ORD_ORDER, "external_reference"
    RequireField db, TBL_ORD_ORDER_LINE, "line_no"

    EnsureAddresses db

    billingAddressId = mAddressBillingCh
    If billingAddressId <= 0 Then
        billingAddressId = ResolveAddressId(db, "BILLING", "Muster Handel AG", "Industriestrasse", "15", "6300", "Zug", "CH")
    End If
...
Private Function ResolveOrderIdByExternalReference(ByVal db As DAO.Database, ByVal ExternalReference As String) As Long
    ResolveOrderIdByExternalReference = LookupLongValue( _
        db, _
        "SELECT TOP 1 [order_id] FROM [" & TBL_ORD_ORDER & "] " & _
        "WHERE [external_reference]=" & SqlText(Trim$(ExternalReference)) & ";", _
        "order_id")
End Function
...
Private Function SalesOrderLineExists(ByVal db As DAO.Database, ByVal OrderId As Long, ByVal LineNo As Long) As Boolean
    SalesOrderLineExists = (LookupLongValue( _
        db, _
        "SELECT TOP 1 [order_line_id] FROM [" & TBL_ORD_ORDER_LINE & "] " & _
        "WHERE [order_id]=" & CStr(OrderId) & _
        " AND [line_no]=" & CStr(LineNo) & ";", _
        "order_line_id") > 0)
End Function

Private Sub EnsureDemoSalesOrderLine( _
    ByVal db As DAO.Database, _
    ...
    If SalesOrderLineExists(db, OrderId, LineNo) Then
        mSkippedExistingCount = mSkippedExistingCount + 1
        Exit Sub
    End If
...
Private Sub UpdateOrderStatusCode(ByVal db As DAO.Database, ByVal OrderId As Long, ByVal OrderStatusCode As String)
    If OrderId <= 0 Then Exit Sub

    ExecSql db, _
        "UPDATE [" & TBL_ORD_ORDER & "] SET [order_status_code]=" & SqlText(UCase$(Trim$(OrderStatusCode))) & _
